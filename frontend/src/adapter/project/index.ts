...
import { MemberAdapter } from '@/adapter/member'
...
export class ProjectAdapter {
  ...
  adapt() {
    return new Project({
      ...
      preview:
        this.data.attributes.preview?.data?.attributes.formats.small.url || '',
      status:
        this.data.attributes.status.data?.attributes.title || 'Не определен',
      address: this.data.attributes.address,
      images:
        this.data.attributes.images?.data?.map(
          (image) => image.attributes.url
        ) || [],
      members:
        this.data.attributes.members?.data?.map((member) =>
          new MemberAdapter(member).adapt()
        ) || [],
